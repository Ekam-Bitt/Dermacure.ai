...
function OtpLogin() {
  ...
  const verifyOtp = async () => {
    console.log("verifyOtp called for OTP:", otp);
  ...
  const requestOtp = async (e?: FormEvent<HTMLFormElement>) => {
    ...
    console.log("requestOtp called for phone number:", phoneNumber);
    ...
    <div className="flex flex-col justify-center items-center">
      {!confirmationResult && (
        <form onSubmit={requestOtp}>
          <Input
            className=" w-full text-black border-2 border-blue-100 rounded-md p-2"
            type="tel"
            value={phoneNumber}
      ...
      <Button
        ...
        onClick={() => requestOtp()}
        className="mt-5 bg-blue-500 text-white rounded-md p-2 w-64"
      >
        {resendCountdown > 0
